...
} = require("../templates email/accoutCreation");
const {
  generateAccountActivationSuccessEmail,
} = require("../templates email/accountVerificationSuccess");
const {
  generateForgotPasswordEmail,
} = require("../templates email/forgotPassword");
const {
  generatePasswordChangeNotificationEmail,
} = require("../templates email/passwordRessetSuccess");
  ...
  try {
    ...
    try {
      await sendMail({
        ...
        html:generateAccountActivationEmail(email, activationUrl)
      });
...
exports.verifyUser = asyncHandler(async (req, res) => {
  try {
    ...
    const emailContent = generateAccountActivationSuccessEmail(
      decoded.username,
      decoded.email,
      decoded.phoneNumber,
      decoded.password,
      decoded.referralLink
    );
    ...
    try {
      await sendMail({
        from: process.env.SMTP_MAIL,
        to: decoded.email,
        subject: "Account Activation Successful and Login Procedure",
        html: emailContent,
      });
    } catch (error) {
...
exports.forgotPassword = asyncHandler(async (req, res) => {
  ...
  // Generate the email content
  const emailContent = generateForgotPasswordEmail(username, resetURL);
...
exports.resetPassword = asyncHandler(async (req, res) => {
  ...
  // Generate the login and forgot password links (you can use your URLs)
  const loginLink = `${process.env.FRONTEND_URL}/login`;
  const forgotPasswordLink = `${process.env.FRONTEND_URL}/forgot-password`;
  const username = user.username;

  // Generate the email content
  const emailContent = generatePasswordChangeNotificationEmail(
    username,
    loginLink,
    forgotPasswordLink
  );
  ...
  return res.status(200).json({
    success: true,
    message: "Password change notification email sent successfully",
  });
});
